...
public class WaterTank : BaseObject, IReceive, IGive
{
    ...
    private void Update()
    {
        ...
        if (timer.Working)
        {
            foreach (GameObject obj in outputs)
            {
                ...
                if (tmp != null)
                {
                    tmp.hardness += maxInfluence / maxTime * Time.deltaTime;
                    tmp.crispness += maxInfluence / maxTime * Time.deltaTime;
                }
            }
        }
    ...
    private bool CheckSynthessisTable(out List<GameObject> ret)
    {
        ...
        string excelSheetName = "WaterTank";
